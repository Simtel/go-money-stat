...
func TestRepository_GetAll(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT `transactions`.`id`,`transactions`.`changed`,`transactions`.`created`,`transactions`.`income_instrument`,`transactions`.`income`,`transactions`.`outcome_instrument`,`transactions`.`outcome`,`transactions`.`date`,`transactions`.`deleted`,`transactions`.`income_account`,`transactions`.`outcome_account`,`transactions`.`comment`,`InAccount`.`id` AS `InAccount__id`,`InAccount`.`title` AS `InAccount__title`,`InAccount`.`balance` AS `InAccount__balance`,`InAccount`.`start_balance` AS `InAccount__start_balance`,`InAccount`.`instrument` AS `InAccount__instrument`,`OutAccount`.`id` AS `OutAccount__id`,`OutAccount`.`title` AS `OutAccount__title`,`OutAccount`.`balance` AS `OutAccount__balance`,`OutAccount`.`start_balance` AS `OutAccount__start_balance`,`OutAccount`.`instrument` AS `OutAccount__instrument`,`InAccount__Currency`.`id` AS `InAccount__Currency__id`,`InAccount__Currency`.`title` AS `InAccount__Currency__title`,`InAccount__Currency`.`short_title` AS `InAccount__Currency__short_title`,`InAccount__Currency`.`symbol` AS `InAccount__Currency__symbol`,`InAccount__Currency`.`rate` AS `InAccount__Currency__rate`,`OutAccount__Currency`.`id` AS `OutAccount__Currency__id`,`OutAccount__Currency`.`title` AS `OutAccount__Currency__title`,`OutAccount__Currency`.`short_title` AS `OutAccount__Currency__short_title`,`OutAccount__Currency`.`symbol` AS `OutAccount__Currency__symbol`,`OutAccount__Currency`.`rate` AS `OutAccount__Currency__rate` FROM `transactions` LEFT JOIN `accounts` `InAccount` ON `transactions`.`income_account` = `InAccount`.`id` LEFT JOIN `accounts` `OutAccount` ON `transactions`.`outcome_account` = `OutAccount`.`id` LEFT JOIN `instruments` `InAccount__Currency` ON `InAccount`.`instrument` = `InAccount__Currency`.`id` LEFT JOIN `instruments` `OutAccount__Currency` ON `OutAccount`.`instrument` = `OutAccount__Currency`.`id` WHERE deleted = ? ORDER BY date ASC")).
		WithArgs(0).
		WillReturnRows(rows)
